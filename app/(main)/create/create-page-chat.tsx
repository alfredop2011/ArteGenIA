...
const GEN_STEPS = [
  ...
  { id: 5, label: "Variantes composición",  sub: "Distribuciones de artistas" },
...
export default function CreatePage() {
    ...
    try {
      ...
      setMessages(prev => {
        const updated = [...prev.slice(0, -1), {
          id: uid(), role: "assistant" as const, type: "text" as const, content: data.message,
        }];
        // Show artist library card only when AI says ready AND flyerType is with_photo
      ...
      setRecentContext(prev =>
        [...prev, { role: "user", content: trimmed }, { role: "assistant", content: data.message }].slice(-8)
      );
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      // CRITICAL: only eventType for atmosphere — NEVER name/date/venue/price in bg prompt
      ...
      // Process artist photos (remove-bg, non-fatal)
              ...
              <div className="relative w-16 h-16">
                <svg className="w-16 h-16 -rotate-90" viewBox="0 0 64 64">
                  ...
                  <circle cx="32" cy="32" r="26" fill="none" stroke={palette.colors[0]} strokeWidth="4"
                    strokeLinecap="round" strokeDasharray={163} strokeDashoffset={163 - 163 * (pct / 100)}
    ...
    return (
      <div className="min-h-screen bg-[#0e0e14] text-white flex flex-col items-center justify-center px-4 py-10">
        <div className="w-full max-w-xl">
          <button onClick={() => setPhase("chat")} className="text-gray-500 hover:text-white text-sm mb-8 transition-colors">
            ← Volver al chat
          </button>
            ...
            <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
              {STYLE_OPTIONS.map(opt => (
                <button key={opt.id} onClick={() => setSelectedStyle(opt.id)}
                  className={`flex flex-col items-center gap-1.5 py-3 px-2 rounded-xl border text-xs font-medium transition-all ${selectedStyle === opt.id ? "border-purple-500 bg-purple-500/15 text-purple-300" : "border-white/8 bg-white/[0.03] text-gray-400 hover:border-white/15"}`}>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {COLOR_PALETTES.map(pal => (
                <button key={pal.id} onClick={() => setSelectedPalette(pal.id)}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all ${selectedPalette === pal.id ? "border-purple-500 bg-purple-500/10" : "border-white/8 bg-white/[0.03] hover:border-white/15"}`}>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              {FORMAT_OPTIONS.map(fmt => (
                <button key={fmt.id} onClick={() => setSelectedFormat(fmt.id)}
                  className={`flex flex-col items-center gap-1 py-3 px-2 rounded-xl border transition-all ${selectedFormat === fmt.id ? "border-yellow-400/50 bg-yellow-400/10 text-yellow-400" : "border-white/8 bg-white/[0.03] text-gray-400 hover:border-white/15"}`}>
          ...
          <div className="bg-white/[0.03] border border-white/8 rounded-2xl p-4 mb-5 space-y-1.5 text-sm text-gray-400">
            ...
            <p className="pt-1 border-t border-white/8">
              🎨 {STYLE_OPTIONS.find(s => s.id === selectedStyle)?.label} · {palette.label} · {FORMAT_OPTIONS.find(f => f.id === selectedFormat)?.label}
            </p>
          </div>
    ...
    <div className="flex h-[calc(100vh-56px)] bg-[#0e0e14] text-white overflow-hidden">
          ...
          {messages.map(msg => {
            ...
            if (msg.type === "artist_library") {
              return (
                <div key={msg.id} className="flex justify-start">
                  <div className="w-7 h-7 rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-xs font-black shrink-0 mr-2 mt-1 self-start">
                    AG
                  </div>
                  <div className="max-w-[85%]">
                    ...
                    {!artistsConfirmed && (
                      <button
                        onClick={() => handleArtistLibraryConfirm(artistsAndLogos)}
                    ...
                    {artistsConfirmed && artistsAndLogos.length > 0 && (
                      <p className="mt-1.5 text-[11px] text-green-400">
                        ✓ {artistsAndLogos.length} elemento{artistsAndLogos.length > 1 ? "s" : ""} añadido{artistsAndLogos.length > 1 ? "s" : ""}
                      </p>
                    )}
                  </div>
                </div>
            ...
            return (
              <div key={msg.id} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                {msg.role === "assistant" && (
                  <div className="w-7 h-7 rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-xs font-black shrink-0 mr-2 mt-0.5 self-start">
                    AG
                  </div>
                )}
        ...
        <div className="border-t border-white/[0.06] bg-[#0e0e14] p-4">
          ...
          {(ctaLabel === "confirm" || ctaLabel === "generate") && !inputBlocked && (
            <button onClick={handleCTA}
              ...
              <span>⚡</span>
              {ctaLabel === "confirm" ? "Confirmar y elegir estilo" : "Elegir estilo y generar"}
            </button>
          )}
        </div>
      </div>
      ...
      <aside className={`hidden lg:flex w-64 shrink-0 border-l border-white/[0.06] bg-[#0c0c12] flex-col transition-opacity duration-300 ${hasData ? "opacity-100" : "opacity-40"}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2.5">
          ...
          {eventData.additionalArtists.length > 0 && <DataPill icon="🎵" value={eventData.additionalArtists.join(", ")} />}
          {eventData.flyerType  && (
