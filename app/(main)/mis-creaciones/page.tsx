...
/**
 * /mis-creaciones — galería personal de assets reutilizables (Fase Z.8).
 *
 * Tabs por tipo:
 *   - Todos
 *   - Fotos sin fondo (de /quitar-fondo)
 *   - Stickers IA (de Capas Mágicas)
 *   - Generadas con IA (de Flux/generador)
 *   - Subidas
 *
 * Acciones por asset:
 *   - Descargar (sin consumir crédito — ya pagaste cuando lo creaste)
 *   - Borrar (delete cascade)
 *   - Copiar URL (futuro)
 *   - Usar en flyer (futuro: drag-to-canvas)
 *
 * Storage bar arriba: muestra X / Y MB usados + CTA Pro si alto %.
 */
...
export default function MisCreacionesPage() {
  ...
  const fetchAssets = useCallback(async () => {
    ...
    try {
      ...
      console.error("[mis-creaciones] fetch failed:", e);
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  const handleDelete = useCallback(async (id: string) => {
    if (!confirm("¿Borrar esta creación? No se puede deshacer.")) return;
    try {
      ...
      void fetchAssets(); // refresca storage info
    } catch {
  ...
  const handleDownload = useCallback(async (asset: Asset) => {
    try {
      ...
      a.href = url;
      a.download = asset.name || `creacion-${asset.id}.png`;
  ...
  // Si no logueado: CTA login
  if (!authLoading && !user) {
    return (
      <main className="min-h-screen bg-[#0a0a14] text-white">
        <div className="max-w-2xl mx-auto px-5 py-20 text-center">
          <h1 className="text-[28px] font-black mb-3">Mis creaciones</h1>
          <p className="text-[14px] text-gray-400 mb-6">
            Inicia sesión para ver tu galería personal de fotos sin fondo, stickers IA y más.
          </p>
          <Link
            href="/"
            className="inline-block px-6 py-3 rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 text-white font-bold text-[13px]"
          >
            Volver al inicio
          </Link>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-5 py-10">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between flex-wrap gap-4 mb-3">
            <div>
              <h1 className="text-[28px] md:text-[36px] font-black">Mis creaciones</h1>
              <p className="text-[12.5px] text-gray-400 mt-1">
                Tus fotos sin fondo, stickers IA y más — listas para reusar en cualquier flyer.
              </p>
            </div>
          </div>
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-1 -mx-5 px-5">
          <FilterTab label="Todos" icon="📂" count={counts.all ?? 0} active={filter === "all"} onClick={() => setFilter("all")} />
...
function EmptyState({ filter }: { filter: FilterType }) {
  return (
    <div className="text-center py-16 px-4">
      ...
      <p className="text-[15px] font-bold mb-1">Aún no tienes creaciones</p>
      <p className="text-[12.5px] text-gray-400 mb-6 max-w-md mx-auto">
        ...
        Se guardarán aquí para que las reuses.
      </p>
