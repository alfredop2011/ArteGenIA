...
type Category = "fiesta" | "conciertos" | "festival" | "clases" | "club" | "corporativo";
type Audience = "academias" | "productoras" | "freelance" | "instituciones" | "agencias" | "colegios";
...
type EventItem = {
  ...
  venue: string;
  neighborhood: string;
  country: string; // id país
  city: string; // id ciudad
  category: Category;
  audience: Audience[]; // "para quién es"
  price: number; // 0 = gratis
  url?: string; // link externo: página del evento / pago
  hasSale?: boolean; // ¿venta online activa? (si undefined, se infiere de `url`)
  cancelled?: boolean; // muestra sello CANCELADO
  image: string; // gradiente CSS o url(...) del flyer
};

const CATEGORIES: Record<Category, { label: string; icon: typeof Music; grad: string }> = {
  fiesta: { label: "Fiesta", icon: PartyPopper, grad: "linear-gradient(135deg,#ec4899,#f97316)" },
  conciertos: { label: "Conciertos", icon: Music, grad: "linear-gradient(135deg,#7c3aed,#ec4899)" },
  festival: { label: "Festival", icon: Tent, grad: "linear-gradient(135deg,#f59e0b,#ef4444)" },
  clases: { label: "Clases de baile", icon: Footprints, grad: "linear-gradient(135deg,#10b981,#84cc16)" },
  club: { label: "Club / Discoteca", icon: Disc3, grad: "linear-gradient(135deg,#6366f1,#0ea5e9)" },
  corporativo: { label: "Corporativo", icon: Briefcase, grad: "linear-gradient(135deg,#0ea5e9,#22c55e)" },
};
...
// Imagen de fondo de la card: flyer real si existe, si no gradiente por categoría.
function bgFor(category: Category, imageUrl?: string | null) {
  return imageUrl ? `url('${imageUrl}') center/cover no-repeat` : CATEGORIES[category].grad;
}
...
function rowToEvent(r: EventRow): EventItem {
  return {
    ...
    image: bgFor(r.category, r.image_url),
  };
}
...
export default function EventosPage() {
  ...
  // Eventos: arrancan con el mock y se reemplazan si hay datos reales.
  ...
  // Cargar eventos publicados desde Supabase. Si la tabla no existe todavía
  // (migración sin correr) o no hay filas, mantenemos el mock para no dejar
  // la agenda vacía. La consulta es pública (RLS permite leer 'published').
  useEffect(() => {
    let cancelled = false;
    (async () => {
