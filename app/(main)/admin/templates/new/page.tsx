...
export default function AdminTemplateCreatorPage() {
  ...
  const handleDelete = async (d: TemplateDraft) => {
    if (!confirm(`¿Eliminar borrador "${d.title}"?`)) return;
      ...
      <div className="border-b border-white/[0.06] bg-[#0e0e14]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-xl sm:text-2xl font-black truncate">Creador de plantillas</h1>
            <p className="text-xs sm:text-sm text-gray-500 mt-0.5">
              Crea borradores y publícalos al catálogo
            </p>
          </div>
          <button
          ...
          >
            ...
            <span className="hidden sm:inline">{creating ? "Creando..." : "Nueva plantilla"}</span>
          ...
          <>
            {drafts.length === 0 && !loading && (
              <div className="text-center py-16 px-4">
                ...
                <h3 className="text-xl font-bold mb-2">No tienes borradores</h3>
                <p className="text-gray-400 mb-6">Crea tu primera plantilla con el botón &quot;Nueva plantilla&quot;.</p>
              </div>
            )}
          ...
          <>
            {published.length === 0 && !loading && (
              <div className="text-center py-16 px-4">
                ...
                <h3 className="text-xl font-bold mb-2">No has publicado plantillas todavía</h3>
                <p className="text-gray-400 mb-6">Termina un borrador y pulsa &quot;Publicar&quot; para que aparezca en el catálogo.</p>
              </div>
            )}
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
              {published.map(p => (
      ...
      <Link href={`/editor/draft-${draft.id}?mode=template-creator`} className="block">
        ...
        <div className="p-2 sm:p-3">
          ...
          <p className="text-gray-500 text-[10px] sm:text-xs mt-0.5 flex items-center gap-1.5">
            {draft.status === "draft" && (<><Clock size={10} strokeWidth={2}/> Borrador</>)}
            {draft.status === "ready" && (<><CheckCircle2 size={10} strokeWidth={2}/> Listo</>)}
            {draft.status === "archived" && (<><ArchiveIcon size={10} strokeWidth={2}/> Archivado</>)}
