 ...
 *
 * NOTA: buscamos el customer por email (no por stripe_customer_id en BD)
 * porque no tenemos esa columna. Funciona porque Checkout crea el customer
 * con el mismo email que usa el user en Supabase. Si en el futuro hay
 * duplicados (mismo email, varias cuentas Stripe), migrar a guardar el
 * customer_id en profiles en el webhook checkout.session.completed.
 */
export async function POST(req: NextRequest) {
  try {
    ...
    // Buscar el Stripe Customer por email. Stripe permite varios customers
    // con el mismo email — cogemos el más reciente que tenga subscription
    // activa o trialing.
    ...
    // Elegir el customer más probable: el que tenga una subscription activa.
    // Si ninguno tiene activa, usar el más reciente (sort desc por created).
    ...
    if (!customerId) {
      // Fallback: customer más reciente — quizá ya canceló pero quiere ver
      // facturas / reactivar.
